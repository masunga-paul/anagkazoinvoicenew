// Global Section Navigation Loader for Livewire & Full Page Transitions
function setupNavigationLoader() {
    ...
    if (!loader && document.body) {
        ...
        loader.className = 'fixed inset-0 z-[9999] bg-white text-zinc-900 flex flex-col items-center justify-center opacity-0 pointer-events-none transition-opacity duration-200 select-none';
        loader.innerHTML = `
...
function showLoader() {
    ...
    const bar = document.getElementById('global-nav-bar');
    const card = document.getElementById('global-nav-card');
    ...
    progressInterval = setInterval(() => {
        ...
        if (current < 85) {
            bar.style.width = (current + Math.floor(Math.random() * 12 + 4)) + '%';
        }
    }, 150);
}
